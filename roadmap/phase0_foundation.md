...
**Goal:** Построить масштабируемую и устойчивую инфраструктуру для гибридного сбора, хранения и первичной обработки данных (API + web scraping). Это фундамент для всех последующих фаз.
...
## Гибридный сбор данных и оптимизация (новое)
- Использовать гибридный подход: официальные API для дружественных/своих аккаунтов, web scraping для массового сбора публичных рилсов.
- Внедрить фильтрацию топ-N рилсов по метаданным для скачивания видео (например, по просмотрам, лайкам, ER).
- Для остальных рилсов собирать только метаданные (описание, лайки, просмотры, дата, автор, хэштеги).
- Реализовать очереди задач, пул прокси, рандомизацию user-agent, задержки между запросами для обхода блокировок.
- Логировать ошибки, капчи, блокировки, реализовать автоматический retry.
- Хранить метаданные в реляционной БД или Data Lake, видео — только для топ-N рилсов в объектном хранилище (S3, MinIO, локально).

---

## Deliverables
- Масштабируемый гибридный сбор метаданных и видео (API + web scraping), оптимизация хранения (только топ-N видео), устойчивость к блокировкам, автоматизация повторных попыток.
